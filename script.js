...
setInterval(() => {
  let date = new Date();
  document.querySelector(".tme").textContent = `${
...
let writeData = (data) => {
  document.querySelector(`.maxTemp`).textContent = `${Math.floor(
    data.main.temp_max
  )}°`;
  document.querySelector(`.minTemp`).textContent = `${Math.floor(
    data.main.temp_min
  )}°`;
  document.querySelector(`.humd`).textContent = `${data.main.humidity}%`;
  document.querySelector(`.clod`).textContent = `${data.clouds.all}%`;
  document.querySelector(`.windSpeed`).textContent = `${Math.floor(
    data.wind.speed
  )}km/h`;
  document.querySelector(`.mainTemp`).textContent = `${Math.floor(
    data.main.temp
  )}°`;
